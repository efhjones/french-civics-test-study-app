"""
Generate practice questions from PDF resources using AWS Bedrock (Claude).

This script:
1. Extracts text from a PDF
2. Uses AWS Bedrock Claude to generate questions based on the content
3. Saves questions to DynamoDB with proper source references

Usage:
    python scripts/generate_questions_from_pdf_bedrock.py path/to/document.pdf --document-name "Document Name" --category "French History" --topic-id "hist-005"
"""
...
def chunk_text(page_texts: Dict[int, str], chunk_size: int = 3000) -> List[Dict[str, Any]]:
    ...
    Returns:
        List of chunks with metadata (page numbers, text)
    """
    chunks = []
    current_chunk = ""
    current_pages = []
...
official questions. If the answer to one (or more) appears in the document, use that as the question and 
...
def main():
    parser = argparse.ArgumentParser(description='Generate questions from PDF using AWS Bedrock')
    ...
    print("=== French Civics Question Generator (AWS Bedrock) ===\n")
    print(f"📄 Document: {args.document_name}")
    print(f"📁 Category: {args.category}")
    print(f"🏷️  Topic ID: {args.topic_id}")
    print(f"🤖 Model: {args.model_id}\n")

    # Extract text from PDF
    page_texts = extract_text_from_pdf(args.pdf_path)
